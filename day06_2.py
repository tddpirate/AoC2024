...
def traverse(initial_map: List[str], initial_guardpos: Coords, initial_guarddir: Coords,) -> Tuple[str, Set[Coords]]:
    ...
    while True:
        if DEBUG2:
            sys.stdout.write(f"Iteration: guard is at {guardpos}, direction {guarddir}. Already visited {len(visited_pos)} positions excluding the current one.\n")
        ...
        if (0 <= nextpos.lind and nextpos.lind < len(MAP)) and (0 <= nextpos.pos and nextpos.pos < len(MAP[0])):
...
if __name__ == "__main__":
        ...
        for (cind,ch) in enumerate(sline):
            if ch in GUARDDIRS:
                if guarddir != Coords(0,0):
                    raise error("More than one guard found", guardpos, guarddir, Coords(lind,cind), GUARDDIRS[ch])
                guardpos = Coords(lind,cind)
                guarddir = GUARDDIRS[ch]
                if DEBUG1:
                    sys.stdout.write(f"Found the guard at {guardpos}, direction {guarddir}.\n")
    ...
    if DEBUG1:
        sys.stdout.write(f"Map has {len(MAP)} lines, line is {len(MAP[0])} positions long.\n")
    ...
    for pos in visited_pos:
        for delta in turn90deg:
            ...
            if (0 <= next_candidate.lind and next_candidate.lind < len(MAP)) and (0 <= next_candidate.pos and next_candidate.pos < len(MAP[0])):
                blockage_candidates.add(next_candidate)
    blockage_candidates.discard(guardpos) # Do not obscure guard's initial position.
    if DEBUG1:
        sys.stdout.write(f"There are {len(blockage_candidates)} obstruction candidates at:\n")
    ...
    for candidate in blockage_candidates:
        ...
        updated_row = row_to_update[:candidate.pos] + "#" + row_to_update[candidate.pos+1:]
        curmap[candidate.lind] = updated_row
        (status, visited_pos) = traverse(curmap, guardpos, guarddir)
        if DEBUG1:
            if counter % 100 == 0:
                sys.stdout.write(f"{counter}/{len(blockage_candidates)}: {candidate=}: {status=} positions={len(visited_pos)}\n")
